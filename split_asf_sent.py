...
def main(argv):
    ...
    *******************************************
       ...
       Usage: split_asf_sent.py [py file donwloaded from ASF]
    ...
    *******************************************
    ...
    with open(main_file) as f:
        for line in f:
            if '.zip' in line:
                lof.append(line.split('[')[-1].split(',')[0].replace(" ", ""))
            else: pass
...
##################
    ...
    for i in lof:
        n+=1
        q = open('./asf_dload_'+str(n)+'.py','w+')
        file_content = asf_vertex(i)
        q.write(file_content)
