...
def main(argv):
    ...
    *******************************************
    ...
    dist = []
    avgInsar = []
    for i in range(len(transectmat['transection'][0][0][0])):
        distRead,transectRead = transectmat['transection'][0][0][0][i]
        dist.append(distRead)
        avgInsar.append(transectRead)

    # avgInsar = np.nanmean(transect,axis=1)
    # stdInSAR = np.nanstd(transect,axis=1)

    # avgInsar = np.nanmean(transect)
    # stdInSAR = np.nanstd(transect)
    # avgInsar = transect
    stdInSAR =  np.linspace(0,0,num=len(transectmat['transection'][0][0][0]),endpoint=True)
    ...
    transect_first = Point(26.76,41.35)
    transect_end = Point(27.09,40.05)
...
################################################################################
    ...
    varModel = []
##1-D space##
    x = np.linspace(-dist,dist2,num=len(transectmat['transection'][0][0][0]),endpoint=True)
    xp = (x/1000.)
    # x = np.linspace(-dist,dist2,num=len(avgInsar),endpoint=True)
    # xp = (x/1000.)
################################################################################
    avgInsar = np.nan_to_num(avgInsar)
    G = np.ones([len(avgInsar),2])
    G[:,0] = xp
    ...
    for d in D:
        for s in V:
            ...
            rms = np.sqrt((sum((residual)**2,0))/len(avgInsar))
...
##PLOT average and standard deviation##
            ...
            ax.plot(xp,transectMm,'o',ms=1,mfc='Black', linewidth='0')
...
##PLOT average and standard deviation##
                ax.plot(xp,transectMm,'o',ms=1,mfc='Black', linewidth='0')
                # for i in np.arange(0.0,1.01,0.01):
                #     ax.plot(xp, (transect-i*stdInSAR)*1000, '-',color='#DCDCDC',alpha=0.5)#,color='#DCDCDC')#'LightGrey')
                # for i in np.arange(0.0,1.01,0.01):
                #     ax.plot(xp, (transect+i*stdInSAR)*1000, '-',color='#DCDCDC',alpha=0.5)#'LightGrey')
                # ax.plot(xp,transect,'r-',label = 'Average velocity')
##PLOT average and standard deviation##
                ...
                ax.plot(xp,(m_c_inv*1000.0),'k-',label=label2)
